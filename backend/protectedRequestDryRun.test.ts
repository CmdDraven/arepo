...
test("default config does not mount dry-run middleware", async () => {
  ...
  assert.equal(status.requestPolicy.dryRunAuditAttemptedCount, 0);
  assert.equal(status.requestPolicy.dryRunAuditAppendCount, 0);
  assert.equal(status.requestPolicy.dryRunRunCount, 0);
  assert.equal(status.requestPolicy.dryRun.configured, false);
  assert.equal(status.requestPolicy.dryRun.mounted, false);
  assert.equal(status.requestPolicy.dryRun.observed.count, 0);
  assert.equal(status.requestPolicy.dryRun.planned.computed, false);
  assert.equal(status.requestPolicy.dryRun.audited.configured, false);
  assert.equal(status.requestPolicy.dryRun.audited.attemptedCount, 0);
  assert.equal(status.requestPolicy.dryRun.audited.appendedCount, 0);
  assert.equal(status.requestPolicy.dryRun.enforced, false);
  assert.equal(status.requestPolicy.enforced, false);
...
test("dry-run canary endpoint reports disabled observation status by default", async () => {
  ...
  assert.equal(body.dryRunAuditAttemptedCount, 0);
  assert.equal(body.dryRunAuditAppendCount, 0);
  assert.equal(body.lastResponsePlan, undefined);
  assert.equal(body.dryRun.configured, false);
  assert.equal(body.dryRun.mounted, false);
  assert.equal(body.dryRun.observed.count, 0);
  assert.equal(body.dryRun.planned.computed, false);
  assert.equal(body.dryRun.audited.configured, false);
  assert.equal(body.dryRun.audited.attemptedCount, 0);
  assert.equal(body.dryRun.audited.appendedCount, 0);
  assert.equal(body.dryRun.enforced, false);
...
test("dry-run canary endpoint reports sanitized mounted dry-run status", async () => {
  ...
  assert.equal(body.dryRun.configured, true);
  assert.equal(body.dryRun.mounted, true);
  assert.equal(body.dryRun.observed.count, 1);
  assert.equal(body.dryRun.observed.lastStatus, "wouldDeny");
  assert.equal(body.dryRun.planned.computed, true);
  assert.equal(body.dryRun.planned.lastResponse?.kind, "unauthenticated");
  assert.equal(body.dryRun.planned.lastResponse?.httpStatus, 401);
  assert.equal(body.dryRun.audited.configured, false);
  assert.equal(body.dryRun.audited.attemptedCount, 0);
  assert.equal(body.dryRun.audited.appendedCount, 0);
  assert.equal(body.dryRun.enforced, false);
...
test("dry-run canary endpoint reports sanitized audit append status", async () => {
  ...
  assert.equal(body.dryRunAuditAttemptedCount, 1);
  ...
  assert.equal(body.dryRun.observed.count, 1);
  assert.equal(body.dryRun.planned.computed, true);
  assert.equal(body.dryRun.audited.configured, true);
  assert.equal(body.dryRun.audited.attemptedCount, 1);
  assert.equal(body.dryRun.audited.appendedCount, 1);
  assert.equal(body.dryRun.audited.lastStatus, "written");
  assert.equal(body.dryRun.enforced, false);
  assert.equal(body.enforcementActive, false);
  assert.equal(body.enforced, false);
  assert.equal(body.protectedModeOperational, false);
...
test("dry-run audit flag alone does nothing unless request-policy dry-run is enabled", async () => {
  ...
  assert.equal(dryRun.dryRunAuditAttemptedCount, 0);
  assert.equal(dryRun.dryRunAuditAppendCount, 0);
  assert.equal(dryRun.dryRun.configured, false);
  assert.equal(dryRun.dryRun.mounted, false);
  assert.equal(dryRun.dryRun.audited.configured, true);
  assert.equal(dryRun.dryRun.audited.attemptedCount, 0);
...
test("explicit dry-run config mounts observation only and calls the protected request pipeline", async () => {
  ...
  assert.equal(dryRun.dryRun.configured, true);
  assert.equal(dryRun.dryRun.mounted, true);
  assert.equal(dryRun.dryRun.observed.count, 1);
  assert.equal(dryRun.dryRun.observed.lastStatus, "wouldDeny");
  assert.equal(dryRun.dryRun.planned.computed, true);
  assert.equal(dryRun.dryRun.planned.lastResponse?.kind, "unauthenticated");
  assert.equal(dryRun.dryRun.audited.configured, false);
  assert.equal(dryRun.dryRun.audited.attemptedCount, 0);
  assert.equal(dryRun.dryRun.audited.appendedCount, 0);
...
test("dry-run audit appends sanitized JSONL events when explicitly enabled", async () => {
  ...
  assert.equal(parsed.events.length, 1);
  assert.equal(dryRun.dryRunAuditConfigured, true);
  assert.equal(dryRun.dryRunAuditAttemptedCount, 1);
  ...
  assert.equal(dryRun.dryRun.audited.appendedCount, 1);
  assert.equal(dryRun.dryRun.audited.lastStatus, "written");
  assert.equal(serialized.includes(bearerMaterial), false);
  assert.equal(serialized.includes(sessionMaterial), false);
...
test("dry-run diagnostics are sanitized and report inactive enforcement", async () => {
  ...
  assert.equal(status.requestPolicy.dryRunAuditConfigured, false);
  assert.equal(status.requestPolicy.dryRun.configured, true);
  assert.equal(status.requestPolicy.dryRun.mounted, true);
  assert.equal(status.requestPolicy.dryRun.observed.count, 2);
  assert.equal(status.requestPolicy.dryRun.planned.computed, true);
  assert.equal(status.requestPolicy.dryRun.planned.lastResponse?.kind, "unauthenticated");
  assert.equal(status.requestPolicy.dryRun.audited.configured, false);
  assert.equal(status.requestPolicy.dryRun.enforced, false);
