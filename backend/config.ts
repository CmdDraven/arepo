...
const APP_DATA_ENV = "MDATLAS_APP_DATA_DIR";

export function configPath(cwd = process.cwd()): string {
  return path.join(cwd, ".mdatlas", "config.json");
}
...
export async function loadConfig(cwd = process.cwd()): Promise<VaultConfigFile> {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Invalid mdAtlas config JSON at ${file}: ${
...
export function resolveAppDataDir(
  ...
  if (isProjectDevDirectory(cwd)) return path.join(cwd, ".mdatlas", "data");
  if (process.platform === "linux" && os.homedir()) {
    return path.join(os.homedir(), ".local", "share", "mdatlas");
  }
  return path.join(cwd, ".mdatlas", "data");
}

async function validateConfig(config: VaultConfigFile, file: string): Promise<void> {
  if (!config || typeof config !== "object") {
    throw new Error(`Invalid mdAtlas config at ${file}: config must be an object`);
  }
  if (!config.node || typeof config.node !== "object") {
    throw new Error(`Invalid mdAtlas config at ${file}: node is required`);
  }
  if (config.node.mode !== "local") {
    throw new Error(`Invalid mdAtlas config at ${file}: only local node mode is supported in V1`);
  }
  if (config.node.apiVersion !== 1) {
    throw new Error(`Invalid mdAtlas config at ${file}: apiVersion must be 1`);
  }
  if (config.appDataDir !== undefined) {
    if (typeof config.appDataDir !== "string" || !config.appDataDir.trim()) {
      throw new Error(`Invalid mdAtlas config at ${file}: appDataDir must be a non-empty string`);
    }
    if (config.appDataDir.includes("\0")) {
      throw new Error(`Invalid mdAtlas config at ${file}: appDataDir cannot contain null bytes`);
    }
  }
  if (!Array.isArray(config.vaults)) {
    throw new Error(`Invalid mdAtlas config at ${file}: vaults must be an array`);
  }

  const seenIds = new Set<string>();
  for (const vault of config.vaults) {
    if (!vault || typeof vault !== "object") {
      throw new Error(`Invalid mdAtlas config at ${file}: each vault must be an object`);
    }
    if (typeof vault.id !== "string" || !/^[a-zA-Z0-9_-]+$/.test(vault.id)) {
      throw new Error(
        `Invalid mdAtlas config at ${file}: vault id must contain only letters, numbers, _ or -`,
      );
    }
    if (seenIds.has(vault.id)) {
      throw new Error(`Invalid mdAtlas config at ${file}: duplicate vault id "${vault.id}"`);
    }
    seenIds.add(vault.id);

    if (typeof vault.displayName !== "string" || !vault.displayName.trim()) {
      throw new Error(`Invalid mdAtlas config at ${file}: vault ${vault.id} needs displayName`);
    }
    if (typeof vault.rootPath !== "string" || !path.isAbsolute(vault.rootPath)) {
      throw new Error(
        `Invalid mdAtlas config at ${file}: vault ${vault.id} rootPath must be absolute`,
      );
    }
    const stat = await fs.stat(vault.rootPath).catch((error) => {
      throw new Error(
        `Invalid mdAtlas config at ${file}: vault ${vault.id} rootPath is not accessible: ${
    ...
    if (!stat.isDirectory()) {
      throw new Error(
        `Invalid mdAtlas config at ${file}: vault ${vault.id} rootPath is not a directory`,
      );
    }

    const permissions = vault.permissions;
    if (!permissions || typeof permissions !== "object") {
      throw new Error(`Invalid mdAtlas config at ${file}: vault ${vault.id} permissions required`);
    }
    for (const key of ["readIndex", "readContent", "writeContent", "deleteFiles"] as const) {
      if (typeof permissions[key] !== "boolean") {
        throw new Error(
          `Invalid mdAtlas config at ${file}: vault ${vault.id} permission ${key} must be boolean`,
        );
      }
    }
    if (!permissions.readIndex) {
      throw new Error(
        `Invalid mdAtlas config at ${file}: vault ${vault.id} must allow readIndex in local mode`,
      );
    }
    if (!permissions.readContent) {
      throw new Error(
        `Invalid mdAtlas config at ${file}: vault ${vault.id} must allow readContent in local mode`,
      );
    }
    if (!permissions.readContent && permissions.writeContent) {
      throw new Error(
        `Invalid mdAtlas config at ${file}: vault ${vault.id} cannot writeContent without readContent`,
      );
    }
    if (permissions.deleteFiles && !permissions.writeContent) {
      throw new Error(
        `Invalid mdAtlas config at ${file}: vault ${vault.id} cannot deleteFiles without writeContent`,
      );
    }
  }
}
...
function isProjectDevDirectory(cwd: string): boolean {
  return path.basename(cwd) === "md_Atlas-main" || process.env.NODE_ENV === "development";
}
