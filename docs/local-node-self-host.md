...
machine, but it has no authentication and must not be exposed to untrusted
networks.
...
Use this only on trusted machines and trusted networks until authentication is
designed.
...
5. If you bind to a LAN address, read the no-auth warning and do not expose the
   backend to untrusted clients.
...
- startup warnings such as non-local binding without authentication
- vault count and per-vault watcher/index health
- storage-summary availability
- explicit capability flags showing that remote nodes, authentication, sync, AI,
  database support, and migration support are disabled in V1

This endpoint does not make LAN or reverse-proxy exposure safe. The backend still
has no authentication, and generated index/cache data remains disposable and
rebuildable from source Markdown files.
