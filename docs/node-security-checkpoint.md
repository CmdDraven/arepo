...
Further custom live browser-auth work is paused pending
[Browser Auth Dependency Evaluation](browser-auth-dependency-evaluation.md).
The current recommendation is to keep protected mode bearer-token based until
AREPO selects a mature auth/session foundation for browser sessions. Existing
...
`backend/betterAuthAppDataStoreProof.ts` now proves that Better Auth can use a
local app-data SQLite database at `app-data/auth/better-auth.sqlite` through
...
`backend/betterAuthExpiredSessionPruningPolicy.ts` now records the
expired-session pruning decision: Better Auth remains authoritative for session
...
paths.
`backend/betterAuthSessionTokenStoragePolicy.ts` now records the storage-policy
decision for Better Auth's stored `session.token` model: acceptable with
...
AREPO now also has test-only browser-auth cookie serialization planning inside
the unmounted dark harness path. That code validates future cookie names,
attributes, redaction, and clearing semantics after internal test-only
pairing/session/CSRF issuance. It is not mounted, does not emit live
`Set-Cookie` headers, does not accept cookies as credentials, and does not
change the live bearer-token protected-mode path.
